...
describe('verifyUserPhone operation', () => {
  ...
  const beforeFunction = (httpClient) => () => {
    jest.clearAllMocks();
    dispatch = jest.fn();
    verifyUserPhone.process({ httpClient, action }, dispatch, jest.fn());
  };

  it('matches snapshot', () => {
    expect(verifyUserPhone).toMatchSnapshot();
  });
  ...
  it('shouldn`t call redirect when redirectRoute isn`t present', () => {
    ...
    verifyUserPhone.process({ httpClient, action: newAction }, dispatch, jest.fn());
