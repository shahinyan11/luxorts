...
import userResendUpdatePhone from '../userResendUpdatePhone';

jest.mock('utils/showErrorNotifications');

describe('userResendUpdatePhone operation', () => {
  let dispatch;

  const beforeFunction = (httpClient) => () => {
    dispatch = jest.fn();
    userResendUpdatePhone.process({ httpClient }, dispatch, jest.fn());
  };

  const { url, endpoint } = userResendUpdatePhoneEndpoint;

  it('matches snapshot', () => {
    expect(userResendUpdatePhone).toMatchSnapshot();
  });
