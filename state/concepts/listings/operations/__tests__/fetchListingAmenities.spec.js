...
import response from 'state/concepts/listings/__mocks__/fetchListingAmenitiesResponse';

import fetchListingAmenities from '../fetchListingAmenities';

jest.mock('utils/showErrorNotifications');

describe('fetchListingAmenities operation', () => {
  let dispatch;
  ...
  const beforeFunction = (httpClient) => () => {
    jest.clearAllMocks();
    dispatch = jest.fn();
    fetchListingAmenities.process({ httpClient, action }, dispatch, jest.fn());
  };

  const { url, endpoint } = fetchListingAmenitiesEndpoint(action.listingId);

  it('has valid attributes', () => {
    expect(fetchListingAmenities).toMatchSnapshot();
  });
