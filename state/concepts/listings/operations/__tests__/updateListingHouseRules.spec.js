...
import updateListingHouseRules from '../updateListingHouseRules';

jest.mock('utils/redirect');
jest.mock('utils/form/assignFormErrors');
jest.mock('utils/showErrorNotifications');

describe('updateListingHouseRules operation', () => {
  let dispatch;
  ...
  const flattenHouseRules = compose(flatten, map(values), values)(action.values.houseRules);
  const { items, ids, relatedIds } = prepareDataForRequest(flattenHouseRules);

  const body = {
    house_rules: [...items, action.values.additional],
  };

  const { endpoint, url } = updateListingHouseRulesEndpoint(action.values.listingId);

  const beforeFunction = (httpClient) => () => {
    jest.clearAllMocks();
    dispatch = jest.fn();
    updateListingHouseRules.process({ httpClient, action }, dispatch, jest.fn());
  };

  it('matches snapshot', () => {
    expect(updateListingHouseRules).toMatchSnapshot();
  });
  ...
  describe('success', () => {
    ...
    it('dispatches actions', () => {
      ...
      expect(dispatch).toHaveBeenNthCalledWith(3, dataDelete({ kind: 'houseRule', ids }));

      expect(dispatch).toHaveBeenNthCalledWith(
        4,
        dataDelete({ kind: 'listingHouseRule', ids: relatedIds }),
      );
  ...
  it('shouldn`t dispatches showMessage when message isn`t present', () => {
    ...
    updateListingHouseRules.process({ httpClient, action: newAction }, dispatch, jest.fn());
