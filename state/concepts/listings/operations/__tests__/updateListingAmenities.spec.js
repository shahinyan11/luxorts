...
import updateListingAmenities from '../updateListingAmenities';

jest.mock('utils/redirect');
jest.mock('utils/form/assignFormErrors');
jest.mock('utils/showErrorNotifications');

describe('updateListingAmenities operation', () => {
  let dispatch;
  ...
  const flattenAmenities = compose(flatten, map(values), values)(action.values.amenities);
  const { items, ids, relatedIds } = prepareDataForRequest(flattenAmenities);

  const body = {
    amenities: [...items, action.values.additional],
  };

  const { endpoint, url } = updateListingAmenitiesEndpoint(action.values.listingId);

  const beforeFunction = (httpClient) => () => {
    jest.clearAllMocks();
    dispatch = jest.fn();
    updateListingAmenities.process({ httpClient, action }, dispatch, jest.fn());
  };

  it('matches snapshot', () => {
    expect(updateListingAmenities).toMatchSnapshot();
  });
  ...
  describe('success', () => {
    ...
    it('dispatches actions', () => {
      ...
      expect(dispatch).toHaveBeenNthCalledWith(3, dataDelete({ kind: 'amenity', ids }));

      expect(dispatch).toHaveBeenNthCalledWith(
        4,
        dataDelete({ kind: 'listingAmenity', ids: relatedIds }),
      );
  ...
  it('shouldn`t dispatches showMessage when message isn`t present', () => {
    ...
    updateListingAmenities.process({ httpClient, action: newAction }, dispatch, jest.fn());
