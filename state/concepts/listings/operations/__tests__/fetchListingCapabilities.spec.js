...
import response from 'state/concepts/listings/__mocks__/fetchCapabilitiesResponse';

import fetchListingCapabilities from '../fetchListingCapabilities';

jest.mock('utils/showErrorNotifications');

describe('fetchListingCapabilities operation', () => {
  let dispatch;
  ...
  const beforeFunction = (httpClient) => () => {
    jest.clearAllMocks();
    dispatch = jest.fn();
    fetchListingCapabilities.process({ httpClient, action }, dispatch, jest.fn());
  };

  const { url, endpoint } = fetchListingCapabilitiesEndpoint(action.listingId);

  it('matches snapshot', () => {
    expect(fetchListingCapabilities).toMatchSnapshot();
  });
