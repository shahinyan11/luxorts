...
import { fetchListingLocationEndpoint } from 'state/concepts/listings/endpoints';
import { dataApiFailure, dataApiRequest, dataApiSuccess } from 'state/data/actions';
import response from 'state/concepts/listings/__mocks__/fetchListingLocationResponse';

import fetchListingLocation from '../fetchListingLocation';

jest.mock('utils/showErrorNotifications');

describe('fetchListingLocation operation', () => {
  let dispatch;
  ...
  const beforeFunction = (httpClient) => () => {
    jest.clearAllMocks();
    dispatch = jest.fn();
    fetchListingLocation.process({ httpClient, action }, dispatch, jest.fn());
  };

  const { url, endpoint } = fetchListingLocationEndpoint(action.listingId);

  it('has valid attributes', () => {
    expect(fetchListingLocation).toMatchSnapshot();
  });
