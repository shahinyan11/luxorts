...
import fetchListingPhotos from '../fetchListingPhotos';

jest.mock('utils/showErrorNotifications');

describe('fetchListingPhotos operation', () => {
  let dispatch;
  ...
  const beforeFunction = (httpClient) => () => {
    jest.clearAllMocks();
    dispatch = jest.fn();
    fetchListingPhotos.process({ httpClient, action }, dispatch, jest.fn());
  };

  const { url, endpoint } = fetchListingPhotosEndpoint(action.listingId);

  it('matches snapshot', () => {
    expect(fetchListingPhotos).toMatchSnapshot();
  });
