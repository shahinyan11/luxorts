...
import fetchListingServices from '../fetchListingServices';

jest.mock('utils/showErrorNotifications');

describe('fetchListingServices operation', () => {
  let dispatch;
  ...
  const beforeFunction = (httpClient) => () => {
    jest.clearAllMocks();
    dispatch = jest.fn();
    fetchListingServices.process({ httpClient, action }, dispatch, jest.fn());
  };

  const { url, endpoint } = fetchListingServicesEndpoint(action.listingId);

  it('matches snapshot', () => {
    expect(fetchListingServices).toMatchSnapshot();
  });
