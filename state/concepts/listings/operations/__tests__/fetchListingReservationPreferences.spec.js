...
import fetchListingReservationPreferences from '../fetchListingReservationPreferences';

jest.mock('utils/showErrorNotifications');

describe('fetchListingReservationPreferences operation', () => {
  let dispatch;
  ...
  const beforeFunction = (httpClient) => () => {
    jest.clearAllMocks();
    dispatch = jest.fn();
    fetchListingReservationPreferences.process({ httpClient, action }, dispatch, jest.fn());
  };

  const { url, endpoint } = fetchListingReservationPreferencesEndpoint(action.listingId);

  it('matches snapshot', () => {
    expect(fetchListingReservationPreferences).toMatchSnapshot();
  });
