...
import response from 'state/concepts/listings/__mocks__/fetchListingPropertyTypeResponse';

import fetchListingPropertyType from '../fetchListingPropertyType';
...
describe('fetchListing operation', () => {
  ...
  const beforeFunction = (httpClient) => () => {
    jest.clearAllMocks();
    dispatch = jest.fn();
    fetchListingPropertyType.process({ httpClient, action }, dispatch, jest.fn());
  };

  const { url, endpoint } = fetchListingPropertyTypeEndpoint(action.listingId);

  it('has valid attributes', () => {
    expect(fetchListingPropertyType).toMatchSnapshot();
  });
