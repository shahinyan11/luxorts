...
import { resendUserEmailVerification } from 'state/concepts/session/actions';
...
describe('EmailVerification useContainer hook', () => {
  let result = null;
  const ctx = {
    store,
    req: {},
  ...
  it('should dispatches resendUserEmailVerification when called onResendClickHandler', () => {
    result.current.onResendClickHandler();

    expect(dispatch).toHaveBeenCalledWith(resendUserEmailVerification());
  });
