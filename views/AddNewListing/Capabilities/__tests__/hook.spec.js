...
jest.mock('state/concepts/listings/selectors', () => ({
  ...
  listingCapabilitiesSelector: jest.fn(() => mockedCapabilities),
}));
...
describe('Capabilities useContainer hook', () => {
  let result = null;
  ...
  it('handleAddNotApprovedCapability method should resets redirectRoute and sets message before submit', async () => {
    await result.current.handleAddNotApprovedCapability();
  ...
  it('handleRemoveNotApprovedCapabilityCreator should calls formik.setFieldValue', () => {
    result.current.handleRemoveNotApprovedCapabilityCreator('id')();

    expect(mockedFormik.setFieldValue).toHaveBeenCalledWith(
      `capabilities.${NOT_APPROVED}.id.checked`,
      false,
    );
  });
  ...
  describe('getInitialProps method', () => {
    ...
    it('should dispatches all actions when user is verified and listingId is present', async () => {
      ...
      expect(store.dispatch).toHaveBeenNthCalledWith(1, dataDeleteEntity('capability'));
      expect(store.dispatch).toHaveBeenNthCalledWith(2, dataDeleteEntity('listingCapability'));
      expect(store.dispatch).toHaveBeenNthCalledWith(3, fetchListingBaseCapabilities());
      expect(store.dispatch).toHaveBeenNthCalledWith(4, fetchListing(mockedListingId));
      expect(store.dispatch).toHaveBeenNthCalledWith(5, fetchListingCapabilities(mockedListingId));
    });
