...
jest.mock('utils/listing/checkListingLocation');

describe('LocationMap useContainer hook', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe('getInitialProps method', () => {
    ...
    it('should calls whenComplete callback twice when is server', async () => {
      await getInitialProps(ctx);

      expect(store.logicMiddleware.whenComplete).toHaveBeenCalledTimes(2);
    });

    it('should calls whenComplete callback once when isn`t server', async () => {
      await getInitialProps({
        ...ctx,
        req: null,
      });

      expect(store.logicMiddleware.whenComplete).toHaveBeenCalledTimes(1);
    });
  });
});
